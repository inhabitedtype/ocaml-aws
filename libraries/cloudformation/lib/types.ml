...
module GetTemplateSummaryOutput = struct
  ...
  let make
      ...
      ?metadata
      () =
    { parameters; description; capabilities; capabilities_reason; version; metadata }
...
module ValidateTemplateOutput = struct
  ...
  let make ?(parameters = []) ?description ?(capabilities = []) ?capabilities_reason () =
...
module GetTemplateOutput = struct
  type t = { template_body : String.t option }

  let make ?template_body () = { template_body }

  let parse xml =
    Some { template_body = Util.option_bind (Xml.member "TemplateBody" xml) String.parse }
