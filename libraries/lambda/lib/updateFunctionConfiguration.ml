open Types
open Aws
type input = UpdateFunctionConfigurationRequest.t
      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render
                  (UpdateFunctionConfigurationRequest.to_query req))))) in
  (`PUT, uri, [])
let of_http body =
  try
    let xml = Ezxmlm.from_string body in
    let resp = Xml.member "UpdateFunctionConfigurationResponse" (snd xml) in
    try
