...
let to_http service region req =
  let uri =
    Uri.add_query_params
      ...
      (List.append
         [("Version", ["2015-03-31"]); ("Action", ["GetEventSourceMapping"])]
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (GetEventSourceMappingRequest.to_query req))))) in
  (`GET, uri, [])
let of_http body =
  try
    let xml = Ezxmlm.from_string body in
    let resp = Xml.member "GetEventSourceMappingResponse" (snd xml) in
    try
      Util.or_error
