open Types
open Aws
type input = UpdateEventSourceMappingRequest.t
      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (UpdateEventSourceMappingRequest.to_query req))))) in
  (`PUT, uri, [])
let of_http body =
  try
    let xml = Ezxmlm.from_string body in
    let resp = Xml.member "UpdateEventSourceMappingResponse" (snd xml) in
    try
      Util.or_error
