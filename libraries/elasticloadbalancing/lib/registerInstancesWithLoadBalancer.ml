  ...
  let uri =
    Uri.add_query_params
      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded (Query.render (RegisterEndPointsInput.to_query req)))))
  in
  `POST, uri, []
...
let of_http body =
  try
    ...
    try
      Util.or_error
        (Util.option_bind resp RegisterEndPointsOutput.parse)
        (let open Error in
        BadResponse
          { body; message = "Could not find well formed RegisterEndPointsOutput." })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing RegisterEndPointsOutput - missing field in body or \
                children: "
               ^ msg
           })
  with Failure msg ->
