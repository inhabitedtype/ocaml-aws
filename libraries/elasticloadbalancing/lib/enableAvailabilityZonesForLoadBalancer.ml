  ...
  let uri =
    Uri.add_query_params
      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded (Query.render (AddAvailabilityZonesInput.to_query req)))))
  in
  `POST, uri, []
...
let of_http body =
  try
    ...
    let resp =
      Util.option_bind
        (Xml.member "EnableAvailabilityZonesForLoadBalancerResponse" (snd xml))
    ...
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing AddAvailabilityZonesOutput - missing field in body or \
                children: "
               ^ msg
           })
  with Failure msg ->
