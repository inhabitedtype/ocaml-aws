...
let to_http service region req =
  let uri =
    Uri.add_query_params
      ...
      (List.append
         [ "Version", [ "2012-06-01" ]; "Action", [ "DescribeLoadBalancerAttributes" ] ]
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (DescribeLoadBalancerAttributesInput.to_query req)))))
  in
  `POST, uri, []
  ...
  try
    ...
    try
      Util.or_error
        ...
        BadResponse
          { body
          ; message = "Could not find well formed DescribeLoadBalancerAttributesOutput."
          })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing DescribeLoadBalancerAttributesOutput - missing field in \
                body or children: "
               ^ msg
           })
  with Failure msg ->
