  ...
  let uri =
    Uri.add_query_params
      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded (Query.render (CreateAccessPointInput.to_query req)))))
  in
  `POST, uri, []
...
let of_http body =
  try
    ...
    let resp =
      Util.option_bind
        (Xml.member "CreateLoadBalancerResponse" (snd xml))
        (Xml.member "CreateLoadBalancerResult")
    in
    try
      Util.or_error
        (Util.option_bind resp CreateAccessPointOutput.parse)
        (let open Error in
        BadResponse
          { body; message = "Could not find well formed CreateAccessPointOutput." })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing CreateAccessPointOutput - missing field in body or \
                children: "
               ^ msg
           })
  with Failure msg ->
...
let parse_error code err =
  let errors =
    ...
    ; Errors_internal.DuplicateLoadBalancerName
    ]
