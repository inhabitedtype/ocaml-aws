  ...
  let uri =
    Uri.add_query_params
      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded (Query.render (GetMetricStatisticsInput.to_query req)))))
  in
  `POST, uri, []
...
let of_http body =
  try
    ...
    let resp =
      Util.option_bind
        (Xml.member "GetMetricStatisticsResponse" (snd xml))
        (Xml.member "GetMetricStatisticsResult")
    in
    try
      Util.or_error
        (Util.option_bind resp GetMetricStatisticsOutput.parse)
        (let open Error in
        BadResponse
          { body; message = "Could not find well formed GetMetricStatisticsOutput." })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing GetMetricStatisticsOutput - missing field in body or \
                children: "
               ^ msg
           })
  with Failure msg ->
