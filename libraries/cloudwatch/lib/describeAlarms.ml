  ...
  let uri =
    Uri.add_query_params
      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded (Query.render (DescribeAlarmsInput.to_query req)))))
  in
  `POST, uri, []
...
let of_http body =
  try
    ...
    let resp =
      Util.option_bind
        (Xml.member "DescribeAlarmsResponse" (snd xml))
        (Xml.member "DescribeAlarmsResult")
    in
    try
      Util.or_error
        (Util.option_bind resp DescribeAlarmsOutput.parse)
        (let open Error in
        BadResponse { body; message = "Could not find well formed DescribeAlarmsOutput." })
