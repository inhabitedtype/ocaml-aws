...
module Event = struct
  ...
  let make
      ...
      ?cloud_trail_event
      () =
    { event_id; event_name; event_time; username; resources; cloud_trail_event }
...
module LookupEventsRequest = struct
  ...
  let make ?(lookup_attributes = []) ?start_time ?end_time ?max_results ?next_token () =
...
module DescribeTrailsRequest = struct
  type t = { trail_name_list : TrailNameList.t }
