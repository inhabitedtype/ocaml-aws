...
let of_http body =
  try
    ...
    try
      Util.or_error (Util.option_bind resp GetInsightSelectorsResponse.parse)
        (let open Error in
           BadResponse
             {
               body;
               message =
                 "Could not find well formed GetInsightSelectorsResponse."
             })
    with
            ...
            (BadResponse
               {
                 body;
                 message =
                   ("Error parsing GetInsightSelectorsResponse - missing field in body or children: "
                      ^ msg)
               })
  with
  | Failure msg ->
