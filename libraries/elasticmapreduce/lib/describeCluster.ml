  ...
  let uri =
    Uri.add_query_params
      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded (Query.render (DescribeClusterInput.to_query req)))))
  in
  `POST, uri, []
...
let of_http body =
  try
    ...
    try
      Util.or_error
        (Util.option_bind resp DescribeClusterOutput.parse)
        (let open Error in
        BadResponse
          { body; message = "Could not find well formed DescribeClusterOutput." })
