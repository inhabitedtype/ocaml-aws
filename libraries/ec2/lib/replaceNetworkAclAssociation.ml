...
let to_http service region (req : input) =
  let uri =
    Uri.add_query_params
      ...
      (List.append
         [ "Version", [ "2015-04-15" ]; "Action", [ "ReplaceNetworkAclAssociation" ] ]
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (ReplaceNetworkAclAssociationRequest.to_query req)))))
  in
  `POST, uri, []
  ...
  try
    ...
    try
      Util.or_error
        ...
        BadResponse
          { body
          ; message = "Could not find well formed ReplaceNetworkAclAssociationResult."
          })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing ReplaceNetworkAclAssociationResult - missing field in body \
                or children: "
               ^ msg
           })
  with Failure msg ->
