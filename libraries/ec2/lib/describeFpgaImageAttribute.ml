      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (DescribeFpgaImageAttributeRequest.to_query req))))) in
  (`POST, uri, [])
let of_http body =
  try
           ...
           BadResponse
             {
               body;
               message =
                 "Could not find well formed DescribeFpgaImageAttributeResult."
             })
    with
            ...
            (BadResponse
               {
                 body;
                 message =
                   ("Error parsing DescribeFpgaImageAttributeResult - missing field in body or children: "
                      ^ msg)
               })
  with
  | Failure msg ->
