      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (DescribeReservedInstancesListingsRequest.to_query req)))))
  in
  `POST, uri, []
...
let of_http body =
  try
    ...
    let resp = Xml.member "DescribeReservedInstancesListingsResponse" (snd xml) in
    try
      Util.or_error
    ...
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing DescribeReservedInstancesListingsResult - missing field in \
                body or children: "
               ^ msg
           })
  with Failure msg ->
