...
let of_http body =
  try
    ...
    let resp =
      Xml.member "RegisterTransitGatewayMulticastGroupSourcesResponse" (snd xml)
    in
    try
      Util.or_error
        (Util.option_bind resp RegisterTransitGatewayMulticastGroupSourcesResult.parse)
        (let open Error in
        BadResponse
          { body
          ; message =
    ...
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing RegisterTransitGatewayMulticastGroupSourcesResult - \
