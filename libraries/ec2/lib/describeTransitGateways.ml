      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (DescribeTransitGatewaysRequest.to_query req)))))
  in
  `POST, uri, []
  ...
  try
    ...
    try
      Util.or_error
        ...
        (let open Error in
        BadResponse
          { body; message = "Could not find well formed DescribeTransitGatewaysResult." })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing DescribeTransitGatewaysResult - missing field in body or \
                children: "
               ^ msg
           })
  with Failure msg ->
