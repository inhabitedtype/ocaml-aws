...
let to_http service region (req : input) =
  let uri =
    Uri.add_query_params
      ...
      (List.append
         [ "Version", [ "2015-04-15" ]; "Action", [ "DescribeSpotFleetRequests" ] ]
         (Util.drop_empty
  ...
  try
    ...
    try
      Util.or_error
        ...
        BadResponse
          { body
          ; message = "Could not find well formed DescribeSpotFleetRequestsResponse."
          })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing DescribeSpotFleetRequestsResponse - missing field in body \
                or children: "
               ^ msg
           })
  with Failure msg ->
