...
let to_http service region (req : input) =
  let uri =
    Uri.add_query_params
      ...
      (List.append
         [ "Version", [ "2015-04-15" ]; "Action", [ "DescribeVpcEndpointServices" ] ]
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (DescribeVpcEndpointServicesRequest.to_query req)))))
  in
  `POST, uri, []
  ...
  try
    ...
    try
      Util.or_error
        ...
        BadResponse
          { body
          ; message = "Could not find well formed DescribeVpcEndpointServicesResult."
          })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing DescribeVpcEndpointServicesResult - missing field in body \
                or children: "
               ^ msg
           })
  with Failure msg ->
