...
let to_http service region (req : input) =
  let uri =
    Uri.add_query_params
      ...
      (List.append
         [ "Version", [ "2015-04-15" ]; "Action", [ "ReplaceRouteTableAssociation" ] ]
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (ReplaceRouteTableAssociationRequest.to_query req)))))
  in
  `POST, uri, []
  ...
  try
    ...
    try
      Util.or_error
        ...
        BadResponse
          { body
          ; message = "Could not find well formed ReplaceRouteTableAssociationResult."
          })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing ReplaceRouteTableAssociationResult - missing field in body \
                or children: "
               ^ msg
           })
  with Failure msg ->
