  ...
  let uri =
    Uri.add_query_params
      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded (Query.render (GetHostedZoneRequest.to_query req)))))
  in
  `GET, uri, []
...
let of_http body =
  try
    ...
    try
      Util.or_error
        (Util.option_bind resp GetHostedZoneResponse.parse)
        (let open Error in
        BadResponse
          { body; message = "Could not find well formed GetHostedZoneResponse." })
