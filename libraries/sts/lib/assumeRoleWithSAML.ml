  ...
  let uri =
    Uri.add_query_params
      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded (Query.render (AssumeRoleWithSAMLRequest.to_query req)))))
  in
  `POST, uri, []
...
let of_http body =
  try
    ...
    let resp =
      Util.option_bind
        (Xml.member "AssumeRoleWithSAMLResponse" (snd xml))
    ...
    try
      Util.or_error
        ...
        (let open Error in
        BadResponse
          { body; message = "Could not find well formed AssumeRoleWithSAMLResponse." })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing AssumeRoleWithSAMLResponse - missing field in body or \
                children: "
               ^ msg
           })
  with Failure msg ->
