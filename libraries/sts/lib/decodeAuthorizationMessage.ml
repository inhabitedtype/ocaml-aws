  ...
  try
    ...
    try
      Util.or_error
        ...
        BadResponse
          { body
          ; message = "Could not find well formed DecodeAuthorizationMessageResponse."
          })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing DecodeAuthorizationMessageResponse - missing field in body \
                or children: "
               ^ msg
           })
  with Failure msg ->
...
let parse_error code err =
  let errors =
    [ Errors_internal.InvalidAuthorizationMessageException ] @ Errors_internal.common
  in
