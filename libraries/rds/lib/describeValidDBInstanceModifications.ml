      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (DescribeValidDBInstanceModificationsMessage.to_query req)))))
  in
  `POST, uri, []
  ...
  try
    ...
    let resp =
      Util.option_bind
        ...
        (Xml.member "DescribeValidDBInstanceModificationsResult")
    in
    try
      Util.or_error
        (Util.option_bind resp DescribeValidDBInstanceModificationsResult.parse)
        (let open Error in
        BadResponse
          { body
          ; message =
    ...
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing DescribeValidDBInstanceModificationsResult - missing field \
                in body or children: "
               ^ msg
           })
  with Failure msg ->
