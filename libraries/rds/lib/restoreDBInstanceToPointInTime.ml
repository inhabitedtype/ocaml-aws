      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (RestoreDBInstanceToPointInTimeMessage.to_query req)))))
  in
  `POST, uri, []
  ...
  try
    ...
    try
      Util.or_error
        ...
        BadResponse
          { body
          ; message = "Could not find well formed RestoreDBInstanceToPointInTimeResult."
          })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing RestoreDBInstanceToPointInTimeResult - missing field in \
                body or children: "
               ^ msg
           })
  with Failure msg ->
