      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (DescribeDBInstanceAutomatedBackupsMessage.to_query req)))))
  in
  `POST, uri, []
  ...
  try
    ...
    let resp =
      Util.option_bind
        ...
        (Xml.member "DescribeDBInstanceAutomatedBackupsResult")
    in
    try
      Util.or_error
        ...
        BadResponse
          { body
          ; message = "Could not find well formed DBInstanceAutomatedBackupMessage."
          })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing DBInstanceAutomatedBackupMessage - missing field in body \
                or children: "
               ^ msg
           })
  with Failure msg ->
