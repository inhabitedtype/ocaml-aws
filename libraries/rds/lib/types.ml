...
module DomainMembership = struct
  ...
  let make ?domain ?status ?connectivity () = { domain; status; connectivity }

  let parse xml =
    Some
      { domain = Util.option_bind (Xml.member "Domain" xml) String.parse
      ; status = Util.option_bind (Xml.member "Status" xml) String.parse
      ; connectivity = Util.option_bind (Xml.member "Connectivity" xml) String.parse
      }

  let to_query v =
    Query.List
      (Util.list_filter_opt
         [ Util.option_map v.connectivity (fun f ->
               Query.Pair ("Connectivity", String.to_query f))
         ; Util.option_map v.status (fun f -> Query.Pair ("Status", String.to_query f))
  ...
  let to_json v =
    `Assoc
      (Util.list_filter_opt
         [ Util.option_map v.connectivity (fun f -> "connectivity", String.to_json f)
         ; Util.option_map v.status (fun f -> "status", String.to_json f)
  ...
  let of_json j =
    ...
    ; status = Util.option_map (Json.lookup j "status") String.of_json
    ; connectivity = Util.option_map (Json.lookup j "connectivity") String.of_json
    }
end
...
module DBClusterMember = struct
  ...
  let make
      ...
      () =
    { d_b_instance_identifier; is_cluster_writer; d_b_cluster_parameter_group_status }
...
module DBClusterParameterGroup = struct
  ...
  let make ?d_b_cluster_parameter_group_name ?d_b_parameter_group_family ?description () =
    { d_b_cluster_parameter_group_name; d_b_parameter_group_family; description }
...
module DBParameterGroup = struct
  ...
  let make ?d_b_parameter_group_name ?d_b_parameter_group_family ?description () =
...
module Event = struct
  ...
  let make ?source_identifier ?source_type ?message ?(event_categories = []) ?date () =
...
module Certificate = struct
  ...
  let make
      ...
      ?valid_till
      () =
    { certificate_identifier; certificate_type; thumbprint; valid_from; valid_till }
...
module DeleteDBInstanceMessage = struct
  ...
  let make ~d_b_instance_identifier ?skip_final_snapshot ?final_d_b_snapshot_identifier ()
      =
...
module InsufficientDomainCapacityFault = struct
  type t = unit
...
module CopyDBSnapshotMessage = struct
  ...
  let make
      ...
      ?copy_tags
      () =
    { source_d_b_snapshot_identifier; target_d_b_snapshot_identifier; tags; copy_tags }
...
module FailoverDBClusterMessage = struct
  type t = { d_b_cluster_identifier : String.t option }
