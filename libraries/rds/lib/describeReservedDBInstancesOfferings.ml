      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (DescribeReservedDBInstancesOfferingsMessage.to_query req)))))
  in
  `POST, uri, []
  ...
  try
    ...
    try
      Util.or_error
        ...
        BadResponse
          { body
          ; message = "Could not find well formed ReservedDBInstancesOfferingMessage."
          })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing ReservedDBInstancesOfferingMessage - missing field in body \
                or children: "
               ^ msg
           })
  with Failure msg ->
...
let parse_error code err =
  let errors =
    [ Errors_internal.ReservedDBInstancesOfferingNotFound ] @ Errors_internal.common
  in
