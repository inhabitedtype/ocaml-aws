  ...
  let uri =
    Uri.add_query_params
      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded (Query.render (ModifyDBInstanceMessage.to_query req)))))
  in
  `POST, uri, []
...
let of_http body =
  try
    ...
    let resp =
      Util.option_bind
        (Xml.member "ModifyDBInstanceResponse" (snd xml))
        (Xml.member "ModifyDBInstanceResult")
    in
    try
      Util.or_error
        (Util.option_bind resp ModifyDBInstanceResult.parse)
        (let open Error in
        BadResponse
          { body; message = "Could not find well formed ModifyDBInstanceResult." })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing ModifyDBInstanceResult - missing field in body or \
                children: "
               ^ msg
           })
  with Failure msg ->
