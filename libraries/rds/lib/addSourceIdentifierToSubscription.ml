      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (AddSourceIdentifierToSubscriptionMessage.to_query req)))))
  in
  `POST, uri, []
  ...
  try
    ...
    let resp =
      Util.option_bind
        ...
        (Xml.member "AddSourceIdentifierToSubscriptionResult")
    in
    try
      Util.or_error
    ...
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing AddSourceIdentifierToSubscriptionResult - missing field in \
                body or children: "
               ^ msg
           })
  with Failure msg ->
