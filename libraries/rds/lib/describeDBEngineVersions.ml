...
let to_http service region req =
  let uri =
    Uri.add_query_params
      ...
      (List.append
         [ "Version", [ "2014-10-31" ]; "Action", [ "DescribeDBEngineVersions" ] ]
         (Util.drop_empty
...
let of_http body =
  try
    ...
    try
      Util.or_error
        (Util.option_bind resp DBEngineVersionMessage.parse)
        (let open Error in
        BadResponse
          { body; message = "Could not find well formed DBEngineVersionMessage." })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing DBEngineVersionMessage - missing field in body or \
                children: "
               ^ msg
           })
  with Failure msg ->
