...
let to_http service region (req : input) =
  let uri =
    Uri.add_query_params
      ...
      (List.append
         [ "Version", [ "2014-10-31" ]; "Action", [ "DescribeOptionGroupOptions" ] ]
         (Util.drop_empty
...
let of_http body =
  try
    ...
    try
      Util.or_error
        (Util.option_bind resp OptionGroupOptionsMessage.parse)
        (let open Error in
        BadResponse
          { body; message = "Could not find well formed OptionGroupOptionsMessage." })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing OptionGroupOptionsMessage - missing field in body or \
                children: "
               ^ msg
           })
  with Failure msg ->
