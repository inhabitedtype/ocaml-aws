...
let of_http body =
  try
    ...
    let resp =
      Util.option_bind
        (Xml.member "CreateDBSubnetGroupResponse" (snd xml))
        (Xml.member "CreateDBSubnetGroupResult")
    in
    try
      Util.or_error
        (Util.option_bind resp CreateDBSubnetGroupResult.parse)
        (let open Error in
        BadResponse
          { body; message = "Could not find well formed CreateDBSubnetGroupResult." })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing CreateDBSubnetGroupResult - missing field in body or \
                children: "
               ^ msg
           })
  with Failure msg ->
...
let parse_error code err =
  let errors =
    ...
    ; Errors_internal.DBSubnetGroupAlreadyExists
    ]
