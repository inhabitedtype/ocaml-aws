...
let to_http service region req =
  let uri =
    Uri.add_query_params
      ...
      (List.append
         [ "Version", [ "2014-10-31" ]; "Action", [ "RestoreDBInstanceFromDBSnapshot" ] ]
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (RestoreDBInstanceFromDBSnapshotMessage.to_query req)))))
  in
  `POST, uri, []
  ...
  try
    ...
    try
      Util.or_error
        ...
        BadResponse
          { body
          ; message = "Could not find well formed RestoreDBInstanceFromDBSnapshotResult."
          })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing RestoreDBInstanceFromDBSnapshotResult - missing field in \
                body or children: "
               ^ msg
           })
  with Failure msg ->
