...
let of_http body =
  try
    ...
    let resp =
      Util.option_bind
        (Xml.member "DescribeCertificatesResponse" (snd xml))
        (Xml.member "DescribeCertificatesResult")
    in
    try
      Util.or_error
        (Util.option_bind resp CertificateMessage.parse)
        (let open Error in
        BadResponse { body; message = "Could not find well formed CertificateMessage." })
