      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (ModifyDBClusterMessage.to_query req)))))
     in
  (`POST, uri, []) 
let of_http body =
  try
...
let parse_error code err =
  let errors =
    ...
    Errors.DBClusterParameterGroupNotFound;
    Errors.InvalidSubnet;
    Errors.InvalidDBSubnetGroupStateFault;
    ...
    Errors.DBClusterNotFoundFault] @ Errors.common  in
