...
functor
  ...
  struct
    ...
    let delete_autoscaling ~auto_scaling_group_name () =
      Runtime.(un_m
                 (run_request
                    ...
                    (Types.DeleteAutoScalingGroupType.make ~auto_scaling_group_name:"aws-test-autoscaling_group" ())
                 ))
    ...
    let create_autoscaling_group_test () =
      ...
      let result = create_autoscaling ~instance_id:(instance_id.Aws_ec2.Types.Instance.instance_id) ~auto_scaling_group_name () in
