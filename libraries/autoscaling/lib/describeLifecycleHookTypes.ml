  ...
  try
    ...
    let resp =
      Util.option_bind
        ...
        (Xml.member "DescribeLifecycleHookTypesResult")
    in
    try
      Util.or_error
        ...
        BadResponse
          { body
          ; message = "Could not find well formed DescribeLifecycleHookTypesAnswer."
          })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing DescribeLifecycleHookTypesAnswer - missing field in body \
                or children: "
               ^ msg
           })
  with Failure msg ->
