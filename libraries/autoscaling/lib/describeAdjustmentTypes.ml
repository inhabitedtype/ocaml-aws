  ...
  try
    ...
    let resp =
      Util.option_bind
        ...
        (Xml.member "DescribeAdjustmentTypesResult")
    in
    try
      Util.or_error
        ...
        (let open Error in
        BadResponse
          { body; message = "Could not find well formed DescribeAdjustmentTypesAnswer." })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing DescribeAdjustmentTypesAnswer - missing field in body or \
                children: "
               ^ msg
           })
  with Failure msg ->
