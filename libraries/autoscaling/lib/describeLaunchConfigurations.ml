      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (LaunchConfigurationNamesType.to_query req)))))
  in
  `POST, uri, []
  ...
  try
    ...
    let resp =
      Util.option_bind
        ...
        (Xml.member "DescribeLaunchConfigurationsResult")
    in
    try
      Util.or_error
        (Util.option_bind resp LaunchConfigurationsType.parse)
        (let open Error in
        BadResponse
          { body; message = "Could not find well formed LaunchConfigurationsType." })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing LaunchConfigurationsType - missing field in body or \
                children: "
               ^ msg
           })
  with Failure msg ->
