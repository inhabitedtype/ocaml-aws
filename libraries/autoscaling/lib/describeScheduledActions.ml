...
let to_http service region req =
  let uri =
    Uri.add_query_params
      ...
      (List.append
         [ "Version", [ "2011-01-01" ]; "Action", [ "DescribeScheduledActions" ] ]
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (DescribeScheduledActionsType.to_query req)))))
  in
  `POST, uri, []
  ...
  try
    ...
    let resp =
      Util.option_bind
        ...
        (Xml.member "DescribeScheduledActionsResult")
    in
    try
      Util.or_error
        (Util.option_bind resp ScheduledActionsType.parse)
        (let open Error in
        BadResponse { body; message = "Could not find well formed ScheduledActionsType." })
