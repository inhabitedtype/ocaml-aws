      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (DescribeScalingActivitiesType.to_query req)))))
  in
  `POST, uri, []
  ...
  try
    ...
    let resp =
      Util.option_bind
        ...
        (Xml.member "DescribeScalingActivitiesResult")
    in
    try
      Util.or_error
        (Util.option_bind resp ActivitiesType.parse)
        (let open Error in
        BadResponse { body; message = "Could not find well formed ActivitiesType." })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing ActivitiesType - missing field in body or children: " ^ msg
           })
  with Failure msg ->
    `Error
      (let open Error in
