  ...
  try
    ...
    let resp =
      Util.option_bind
        ...
        (Xml.member "DescribeScalingProcessTypesResult")
    in
    try
      Util.or_error
        (Util.option_bind resp ProcessesType.parse)
        (let open Error in
        BadResponse { body; message = "Could not find well formed ProcessesType." })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing ProcessesType - missing field in body or children: " ^ msg
           })
  with Failure msg ->
    `Error
      (let open Error in
