      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render
                  (DeleteGlobalReplicationGroupMessage.to_query req))))) in
  (`POST, uri, [])
let of_http body =
  try
    ...
    let resp =
      Util.option_bind
        ...
        (Xml.member "DeleteGlobalReplicationGroupResult") in
    try
      Util.or_error
           ...
           BadResponse
             {
               body;
               message =
                 "Could not find well formed DeleteGlobalReplicationGroupResult."
             })
    with
            ...
            (BadResponse
               {
                 body;
                 message =
                   ("Error parsing DeleteGlobalReplicationGroupResult - missing field in body or children: "
                      ^ msg)
               })
  with
  | Failure msg ->
