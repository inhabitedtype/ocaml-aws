...
let to_http service region req =
  let uri =
    Uri.add_query_params
      ...
      (List.append
         [ "Version", [ "2015-02-02" ]; "Action", [ "DescribeReplicationGroups" ] ]
         (Util.drop_empty
...
let of_http body =
  try
    ...
    try
      Util.or_error
        (Util.option_bind resp ReplicationGroupMessage.parse)
        (let open Error in
        BadResponse
          { body; message = "Could not find well formed ReplicationGroupMessage." })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing ReplicationGroupMessage - missing field in body or \
                children: "
               ^ msg
           })
  with Failure msg ->
