      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (ResetCacheParameterGroupMessage.to_query req)))))
     in
  (`POST, uri, []) 
let of_http body =
  try
    ...
    let resp =
      Util.option_bind
        ...
        (Xml.member "ResetCacheParameterGroupResult")
       in
    try
      Util.or_error
...
let parse_error code err =
  let errors =
    ...
    Errors_internal.InvalidCacheParameterGroupState] @ Errors_internal.common
     in
