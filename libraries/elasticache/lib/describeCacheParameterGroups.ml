...
let to_http service region (req : input) =
  let uri =
    Uri.add_query_params
      ...
      (List.append
         [ "Version", [ "2015-02-02" ]; "Action", [ "DescribeCacheParameterGroups" ] ]
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (DescribeCacheParameterGroupsMessage.to_query req)))))
  in
  `POST, uri, []
  ...
  try
    ...
    let resp =
      Util.option_bind
        ...
        (Xml.member "DescribeCacheParameterGroupsResult")
    in
    try
      Util.or_error
        ...
        (let open Error in
        BadResponse
          { body; message = "Could not find well formed CacheParameterGroupsMessage." })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing CacheParameterGroupsMessage - missing field in body or \
                children: "
               ^ msg
           })
  with Failure msg ->
