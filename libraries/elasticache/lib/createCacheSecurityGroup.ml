...
let to_http service region req =
  let uri =
    Uri.add_query_params
      ...
      (List.append
         [ "Version", [ "2015-02-02" ]; "Action", [ "CreateCacheSecurityGroup" ] ]
         (Util.drop_empty
  ...
  try
    ...
    let resp =
      Util.option_bind
        ...
        (Xml.member "CreateCacheSecurityGroupResult")
    in
    try
      Util.or_error
        ...
        (let open Error in
        BadResponse
          { body; message = "Could not find well formed CreateCacheSecurityGroupResult." })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing CreateCacheSecurityGroupResult - missing field in body or \
                children: "
               ^ msg
           })
  with Failure msg ->
...
let parse_error code err =
  let errors =
    ...
    ; Errors_internal.CacheSecurityGroupAlreadyExists
    ]
