  ...
  let uri =
    Uri.add_query_params
      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded (Query.render (CopySnapshotMessage.to_query req)))))
  in
  `POST, uri, []
...
let of_http body =
  try
    ...
    let resp =
      Util.option_bind
        (Xml.member "CopySnapshotResponse" (snd xml))
        (Xml.member "CopySnapshotResult")
    in
    try
      Util.or_error
        (Util.option_bind resp CopySnapshotResult.parse)
        (let open Error in
        BadResponse { body; message = "Could not find well formed CopySnapshotResult." })
