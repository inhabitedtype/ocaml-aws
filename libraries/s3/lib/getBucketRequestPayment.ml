      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (GetBucketRequestPaymentRequest.to_query req)))))
  in
  `GET, uri, []
  ...
  try
    ...
    try
      Util.or_error
        ...
        (let open Error in
        BadResponse
          { body; message = "Could not find well formed GetBucketRequestPaymentOutput." })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing GetBucketRequestPaymentOutput - missing field in body or \
                children: "
               ^ msg
           })
  with Failure msg ->
