...
let of_http body =
  try
    ...
    try
      Util.or_error
        (Util.option_bind resp GetBucketVersioningOutput.parse)
        (let open Error in
        BadResponse
          { body; message = "Could not find well formed GetBucketVersioningOutput." })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing GetBucketVersioningOutput - missing field in body or \
                children: "
               ^ msg
           })
  with Failure msg ->
