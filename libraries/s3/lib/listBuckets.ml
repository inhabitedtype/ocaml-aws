...
let to_http service region (req : input) =
  let uri =
    Uri.add_query_params
      ...
      (List.append
         [ "Version", [ "2006-03-01" ]; "Action", [ "ListBuckets" ] ]
         (Util.drop_empty
...
let of_http body =
  try
    ...
    try
      Util.or_error
        (Util.option_bind resp ListBucketsOutput.parse)
        (let open Error in
        BadResponse { body; message = "Could not find well formed ListBucketsOutput." })
