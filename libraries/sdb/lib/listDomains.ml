...
let to_http service region req =
  let uri =
    Uri.add_query_params
      ...
      (List.append
         [ "Version", [ "2009-04-15" ]; "Action", [ "ListDomains" ] ]
         (Util.drop_empty
            (Uri.query_of_encoded (Query.render (ListDomainsRequest.to_query req)))))
  in
  `POST, uri, []
...
let of_http body =
  try
    ...
    let resp =
      Util.option_bind
        (Xml.member "ListDomainsResponse" (snd xml))
        (Xml.member "ListDomainsResult")
    in
    try
      Util.or_error
        (Util.option_bind resp ListDomainsResult.parse)
        (let open Error in
        BadResponse { body; message = "Could not find well formed ListDomainsResult." })
