  ...
  let uri =
    Uri.add_query_params
      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded (Query.render (SendMessageRequest.to_query req)))))
  in
  `POST, uri, []
...
let of_http body =
  try
    ...
    let resp =
      Util.option_bind
        (Xml.member "SendMessageResponse" (snd xml))
        (Xml.member "SendMessageResult")
    in
    try
      Util.or_error
        (Util.option_bind resp SendMessageResult.parse)
        (let open Error in
        BadResponse { body; message = "Could not find well formed SendMessageResult." })
