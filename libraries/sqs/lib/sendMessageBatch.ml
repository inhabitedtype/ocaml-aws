  ...
  let uri =
    Uri.add_query_params
      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded (Query.render (SendMessageBatchRequest.to_query req)))))
  in
  `POST, uri, []
...
let of_http body =
  try
    ...
    let resp =
      Util.option_bind
        (Xml.member "SendMessageBatchResponse" (snd xml))
        (Xml.member "SendMessageBatchResult")
    in
    try
      Util.or_error
        (Util.option_bind resp SendMessageBatchResult.parse)
        (let open Error in
        BadResponse
          { body; message = "Could not find well formed SendMessageBatchResult." })
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing SendMessageBatchResult - missing field in body or \
                children: "
               ^ msg
           })
  with Failure msg ->
